document.addEventListener("DOMContentLoaded", () => {
  ...
  navItems.forEach((link) => {
    ...
    if (linkPath === currentPage || (currentPage === "index.html" && (linkPath === "/" || linkPath === "index.html"))) {
...
async function loadHomepageContent() {
  const promoBlocksContainer = document.getElementById("promo-blocks-container")
  ...
  if (!content) {
    ...
    if (promoBlocksContainer) promoBlocksContainer.innerHTML = "<p>Error loading promotional content.</p>"
    if (featuredEventsContainer) featuredEventsContainer.innerHTML = "<p>Error loading featured events.</p>"
    if (newsContainer) newsContainer.innerHTML = "<p>Error loading news.</p>" // This should replace "Loading news..."
    if (gameGuidesContainer) gameGuidesContainer.innerHTML = "<p>Error loading game guides.</p>"
    return
  }
  ...
  if (promoBlocksContainer) {
    if (content.promoBlocks && content.promoBlocks.length > 0) {
        ...
        .map(
          (block) => `
              <div class="promo-block">
                  <img src="${block.image || "/placeholder.svg?width=300&height=200"}" alt="${block.title}">
                  <h3>${block.title}</h3>
                  <p>${block.text}</p>
              </div>
          `,
      ...
      promoBlocksContainer.innerHTML = "<p>No promotional content available.</p>"
    }
  }
  ...
  if (featuredEventsContainer) {
    if (content.featuredEvents && content.featuredEvents.length > 0) {
        ...
        .map(
          (event) => `
              <div class="featured-event-card">
                  <img src="${event.image || "/placeholder.svg?width=600&height=400"}" alt="${event.title}">
                  <div class="featured-event-content">
                      <h3>${event.title}</h3>
                      <p class="event-location-date">${event.location} - ${event.date}</p>
                      <p>${event.description}</p>
                      <a href="${event.detailsLink}" class="cta-button">Event Details</a>
                  </div>
              </div>
          `,
        )
      ...
      featuredEventsContainer.innerHTML = "<p>No featured events available.</p>"
    }
  }
          ...
          (item) => `
              <div class="news-item-card">
                  ...
                  <div class="news-item-content">
                      ...
                      <p>${item.snippet}</p>
                      <a href="news-detail.html?id=${item.id}" class="text-link">Read More &rarr;</a>
                  </div>
              </div>
          `,
        )
      ...
      .map(
        (hotel) => `
            <div class="hotel-card">
                <img src="${hotel.images[0] || "/placeholder.svg?width=300&height=200"}" alt="${hotel.name}">
                <div class="hotel-card-content">
                    <h3>${hotel.name}</h3>
                    <p>${hotel.shortDescription || hotel.description.substring(0, 100) + "..."}</p>
                    <a href="hotel.html?id=${hotel.id}" class="cta-button">View Details</a>
                </div>
            </div>
        `,
      )
  ...
  if (hotel) {
    ...
    if (hotel.gamingFacilities && hotel.gamingFacilities.length > 0) {
      gamingFacilitiesHTML = `
                ...
                <ul>${hotel.gamingFacilities.map((facility) => `<li>${facility}</li>`).join("")}</ul>
            `
    }
    let amenitiesHTML = ""
    ...
    hotelDetailContent.innerHTML = `
            ...
            <div class="hotel-image-gallery">
                ${hotel.images.map((img) => `<img src="${img}" alt="${hotel.name} image">`).join("")}
            </div>
    ...
    hotelDetailContent.innerHTML = "<p>Hotel not found or error loading data.</p>"
    eventsContainer.innerHTML = ""
  }
}
...
async function loadHotelEvents(hotelId, eventIds) {
  ...
  const hotelEvents = allEvents.filter(
    (event) => event.hotelId === hotelId || (eventIds && eventIds.includes(event.id)),
  )
  ...
  if (hotelEvents.length > 0) {
        ...
        (event) => `
            <div class="event-card">
                <div class="event-card-content">
                    ...
                    <button class="cta-button view-event-details" data-event-id="${event.id}">View Full Details</button>
                </div>
            </div>
        `,
      )
      ...
      button.addEventListener("click", async () => {
        ...
        if (clickedEvent) {
          ...
          modalEventDetail.innerHTML = `
                        ...
                        <p><strong>Date:</strong> ${new Date(clickedEvent.date).toLocaleDateString()}</p>
    ...
    eventsContainer.innerHTML = "<p>No upcoming events listed for this hotel.</p>"
  }
}
...
async function loadPromotions() {
  ...
  if (promotions && promotions.length > 0) {
      ...
      .map(
        (promo) => `
            <div class="promotion-card">
                <img src="${promo.image || "/placeholder.svg?width=400&height=300"}" alt="${promo.title}">
                <div class="promotion-card-content">
                    <h3>${promo.title}</h3>
                    <p>${promo.description}</p>
                    ${promo.hotelLogo ? `<img src="${promo.hotelLogo}" alt="Hotel Logo" class="hotel-logo">` : ""}
                </div>
            </div>
        `,
      )
    ...
    promotionsContainer.innerHTML = "<p>No holiday ideas found or error loading data.</p>"
  }
}
    ...
    if (content.sections && content.sections.length > 0) {
      content.sections.forEach((section) => {
        ...
        if (section.points && section.points.length > 0) {
          sectionsHTML += `<ul>${section.points.map((point) => `<li>${point}</li>`).join("")}</ul>`
        }
      })
    }
    mainContentContainer.innerHTML = sectionsHTML || "<p>Content loading error.</p>"
  } else {
    if (headerContainer) headerContainer.innerHTML = '<section class="page-header"><h1>About Us</h1></section>'
...
function handleContactForm() {
  ...
  if (form && submissionMessage) {
    form.addEventListener("submit", (e) => {
      e.preventDefault()
      submissionMessage.textContent = "Thank you for your message! (This is a demo, no data was sent)"
...
async function loadNewsDetail() {
  ...
  if (article) {
    ...
    articleContainer.innerHTML = `
            <h1>${article.title}</h1>
            <p class="news-date-full">Published: ${new Date(article.date).toLocaleDateString()}</p>
            ${article.image ? `<img src="${article.image}" alt="${article.title}" class="news-article-image-full">` : ""}
